								...
								<c:forEach items="${selectedCourseList}" var="item">
									<tr>
										...
										<c:if test="${!item.over}">
											<td>Not Marked</td>
											<td>
												<button class="btn btn-default btn-xs btn-info" onClick="location.href='${pageContext.request.contextPath}/teacher/mark?studentid=${item.studentid}&courseid=${item.courseid}'">Mark</button>
											</td>
										</c:if>
										<c:if test="${item.over}">
											<td>${item.mark}</td>
